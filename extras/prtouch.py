...
class PRTouchCFG:
    def __init__(self, config):
        ...
        self.bed_soak_seconds = config.getint('bed_soak_seconds', default=0, minval=0, maxval=7200)
        self.pla_s_hot_min_temp = config.getfloat('pla_s_hot_min_temp', default=self.hot_min_temp, minval=80, maxval=240)
        self.pla_s_hot_max_temp = config.getfloat('pla_s_hot_max_temp', default=self.hot_max_temp, minval=180, maxval=300)
        self.pla_s_bed_max_temp = config.getfloat('pla_s_bed_max_temp', default=self.bed_max_temp, minval=45, maxval=130)
        self.pla_bed_soak_seconds = config.getint('pla_bed_soak_seconds', default=self.bed_soak_seconds, minval=0, maxval=7200)
        self.petg_s_hot_min_temp = config.getfloat('petg_s_hot_min_temp', default=self.hot_min_temp, minval=80, maxval=240)
        self.petg_s_hot_max_temp = config.getfloat('petg_s_hot_max_temp', default=self.hot_max_temp, minval=180, maxval=300)
        self.petg_s_bed_max_temp = config.getfloat('petg_s_bed_max_temp', default=self.bed_max_temp, minval=45, maxval=130)
        self.petg_bed_soak_seconds = config.getint('petg_bed_soak_seconds', default=self.bed_soak_seconds, minval=0, maxval=7200)
        self.tpu_s_hot_min_temp = config.getfloat('tpu_s_hot_min_temp', default=self.hot_min_temp, minval=80, maxval=240)
        self.tpu_s_hot_max_temp = config.getfloat('tpu_s_hot_max_temp', default=self.hot_max_temp, minval=180, maxval=300)
        self.tpu_s_bed_max_temp = config.getfloat('tpu_s_bed_max_temp', default=self.bed_max_temp, minval=45, maxval=130)
        self.tpu_bed_soak_seconds = config.getint('tpu_bed_soak_seconds', default=self.bed_soak_seconds, minval=0, maxval=7200)
    ...
    def get_material_profile(self, material=None):
        material = str(material).strip().lower() if material else ''
        if material == 'pla':
            return {
                'hot_min_temp': self.pla_s_hot_min_temp,
                'hot_max_temp': self.pla_s_hot_max_temp,
                'bed_max_temp': self.pla_s_bed_max_temp,
                'bed_soak_seconds': self.pla_bed_soak_seconds,
            }
        if material == 'petg':
            return {
                'hot_min_temp': self.petg_s_hot_min_temp,
                'hot_max_temp': self.petg_s_hot_max_temp,
                'bed_max_temp': self.petg_s_bed_max_temp,
                'bed_soak_seconds': self.petg_bed_soak_seconds,
            }
        if material == 'tpu':
            return {
                'hot_min_temp': self.tpu_s_hot_min_temp,
                'hot_max_temp': self.tpu_s_hot_max_temp,
                'bed_max_temp': self.tpu_s_bed_max_temp,
                'bed_soak_seconds': self.tpu_bed_soak_seconds,
            }
        return {
            'hot_min_temp': self.hot_min_temp,
            'hot_max_temp': self.hot_max_temp,
            'bed_max_temp': self.bed_max_temp,
            'bed_soak_seconds': self.bed_soak_seconds,
        }
...
class PRTouchZOffsetWrapper:
    ...
    def _check_trigger(self, arg_index, fit_vals, unfit_vals, min_hold, max_hold):
        ...
        if len(fit_vals) >= (self.cfg.pi_count / 2) and math.fabs(fit_vals[-1]) >= max_hold and math.fabs(fit_vals[-2]) >= max_hold and math.fabs(fit_vals[-3]) >= max_hold:
    ...
    def probe_calibrate_finalize(self, kin_pos):
        ...
        gcode.respond_info('%s: z_offset: %.3f
...
with the above and restart the printer.' % (probe_name, z_offset))
    ...
    def cmd_NOZZLE_CLEAR(self, gcmd):
        ...
        prof = self.cfg.get_material_profile(material)
...
def cmd_PRTOUCH_PROBE_ZOFFSET(self, gcmd):
    ...
    def cmd_PRTOUCH_ACCURACY(self, gcmd):
        ...
        gcmd.respond_info('PRTOUCH_ACCURACY at X:%.3f Y:%.3f (samples=%d speed=%.1f)
' % (self.cfg.sensor_x, self.cfg.sensor_y, sample_count, speed))
        ...
        while len(positions) < sample_count:
            ...
            gcmd.respond_info('probe #%d at (%.3f, %.3f): %.3f
