...
def wait_for_temps(bed_target, hotend_target, tol=0.8, soak=15, timeout=1200):
    start = time.time()
    reached_at = None
    last = None
    ...
    raise RuntimeError(
        f"Timeout esperando temperatura estable. "
        f"Bed={last['bed_temp']}/{bed_target}, Hotend={last['ext_temp']}/{hotend_target}"
    )
...
def main():
    ...
    try:
        ...
        wait_for_temps(args.bed, args.hotend)
